# -*- coding: utf-8 -*-
"""
Created on Sat Jun 9 20:38:33 2019
...
thead = pd.read_csv("data/WISDM_ar_v1.1_prep.csv", nrows=5) # just read in a few lines to get the column headers
...
data = pd.read_csv("data/WISDM_ar_v1.1_prep.csv", header=None, skiprows=0, dtype=dtypes, names=column_names).dropna()
...
"""
# Train the model
# This code was used to initially train the model, before loading weights.
...
# Test the model
model.load_state_dict(torch.load("./wisdm_weights/verif_98.452acc"))
model.eval()
with torch.no_grad():
...
# Confusion Matrix
    matestX = np.argmax(y_test, axis=1)
    max_predict = np.argmax(predictions, axis=1)
    confusionM = metrics.confusionM(matestX, max_predict)

    plt.figure(figsize=(16, 14))
    sns.heatmap(confusionM/(np.sum(confusionM, axis=1, keepdims=1)), xticklabels=labels, yticklabels=labels, annot=true);
