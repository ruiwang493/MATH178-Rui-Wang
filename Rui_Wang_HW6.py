...
def normalize(v: np.ndarray) -> np.ndarray: 
    ...
    norm=0
    for i in v:
        norm+=math.sqrt(float(i))
    return v/norm


# Normalize df_pick3 by row
vecList=np.array([0,0,1])
#for i in df:
   # print(i)
    #np.append(vecList, normalize(i))

# create a 3d plot
for i in range(0, len(df["X_gyro"])):
    np.append(vecList, normalize(np.array([df["X_gyro"][i], df["Y_gyro"][i], df["Z_gyro"]])))
